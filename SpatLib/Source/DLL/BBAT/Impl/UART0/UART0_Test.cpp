#include "StdAfx.h"
#include "UART0_Test.h"

//
IMPLEMENT_RUNTIME_CLASS(CUART0_Test)

//////////////////////////////////////////////////////////////////////////
CUART0_Test::CUART0_Test(void)
: m_dwUartNo(0)
, m_dwBaudrate(115200)
, m_iSleepTime(1000)
{
...
SPRESULT CUART0_Test::__PollAction(void)
{
	...
	{
		ReleaseChannel(pCom);
		NOTIFY("Open_UART0", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL,"Port = %d; Baudrate = %d bps", m_dwUartNo, m_dwBaudrate);   
		return SP_E_BBAT_OPEN_CHANNEL_FAIL;
	}
	else
	{
		NOTIFY("Open_UART0", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL,"Port = %d; Baudrate = %d bps", m_dwUartNo, m_dwBaudrate); 
	}
    ...
    LPCSTR STRING = "echo 0 > /proc/sys/kernel/printk\r\n";
	...
	{
		if(memcmp(pFind,"Password:",strlen("Password:")) == 0)
		{
			bFind = true; //找到了
		}
		pFind++;
	...
	if(bFind)
	{
		NOTIFY("CheckUART0", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL,  "RecvData = %s", szRevBuf);  //pass
	}
	else
	{		
		NOTIFY("CheckUART0", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL,  "RecvData = %s", szRevBuf);   
		return SP_E_BBAT_UART_VALUE_FAIL;
	}

    return SP_OK;
}
BOOL CUART0_Test::LoadXMLConfig(void)
{
	m_dwUartNo = GetConfigValue(L"Option:UART", 0);
	m_dwBaudrate = GetConfigValue(L"Option:Baudrate", 115200);
	m_iSleepTime = GetConfigValue(L"Option:RecvTime", 1000);
	return TRUE;
}
