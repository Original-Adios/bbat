...
//
IMPLEMENT_RUNTIME_CLASS(CRelay)
...
SPRESULT CRelay::__PollAction(void)
{
	...
	if(m_bRelay1)
	{
		unsigned char open[] = {0xaa,1,1,};
		unsigned char close[] = {0xaa,1,0};
		if(m_bRelay1_Open)
		{
			pCom->Clear();
			res = Send(pCom, open);
			Sleep(3000);
			NOTIFY("Relay1_Open", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, NULL); 
		}
		if(m_bRelay1_Close)
		{
			pCom->Clear();
			res = Send(pCom, close);
			res = Send(pCom, close);
			NOTIFY("Relay1_Close", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, NULL); 
		}
	}
	if(m_bRelay2)
	{
		unsigned char open[] = {0xaa,2,1,};
		unsigned char close[] = {0xaa,2,0};
		if(m_bRelay2_Open)
		{
			pCom->Clear();
			res = Send(pCom, open);
			Sleep(3000);
			NOTIFY("Relay2_Open", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, NULL); 
		}
		if(m_bRelay2_Close)
		{
			pCom->Clear();
			res = Send(pCom, close);
			res = Send(pCom, close);
			NOTIFY("Relay2_Close", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, NULL); 
		}
	}
	pCom->Close();
	ReleaseChannel(pCom);
...
BOOL CRelay::LoadXMLConfig(void)
{
	...
	m_bRelay1_Open = GetConfigValue(L"Option:Relay1:Open", 0);
	m_bRelay1_Close = GetConfigValue(L"Option:Relay1:Close", 0);
	m_bRelay2_Open = GetConfigValue(L"Option:Relay2:Open", 0);
	m_bRelay2_Close = GetConfigValue(L"Option:Relay2:Close", 0);
	return TRUE;
}

SPRESULT CRelay::Send(ICommChannel* pCom, unsigned char* lpData)
{
...
SPRESULT CRelay::RecvAndCompare(ICommChannel* pCom,LPCSTR lpCompareData)
{
	CHAR szRevBuf[100] = {0};
