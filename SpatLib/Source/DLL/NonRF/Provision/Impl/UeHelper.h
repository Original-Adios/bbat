...
public:
	...
	SPRESULT WriteMGBSN(LPSTR lpSN);

	SPRESULT WriteEthernetMAC(LPSTR lpMac);

	SPRESULT ReadSN(SN_TYPE eSN, LPSTR lpSN);

	SPRESULT ReadProductData(PC_PRODUCT_DATA* pData);

	SPRESULT ReadGEID(PC_GEID_T* pGEID);

	SPRESULT ReadBTAddrByAT(LPSTR lpAddr);
	SPRESULT ReadWIFIAddrByAT(LPSTR lpAddr);

	SPRESULT ReadEthernetMAC(LPSTR lpMac);
