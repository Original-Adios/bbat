...
IMPLEMENT_RUNTIME_CLASS(CEthernet_CheckCodes)

CEthernet_CheckCodes::CEthernet_CheckCodes(void)
{
...
BOOL CEthernet_CheckCodes::LoadXMLConfig(void)
{
    CHKRESULT(__super::LoadXMLConfig());
    return TRUE;
}

SPRESULT CEthernet_CheckCodes::__PollAction(void)
{
	...
	//guanglu.pan 20190906 revised, only read wifi mac
	if (m_InputSN[BC_ENETMAC].bEnable)
	{
		CHKRESULT(ue.ReadEthernetMAC(m_CheckCodes[BC_ENETMAC].szCheckCodesInDUT));
	}
	//
