...
{ 
    ...
    {   
        ...
        CAppVer theVer;
        theVer.Init(hApp);
        LPCWSTR lpszAppVer = theVer.GetVersion();
        m_strAppVer = (lpszAppVer != nullptr) ? lpszAppVer : L"Unknown";

        LPCWSTR lpszVendor = theVer.GetVendor();
        m_strVendor = (lpszVendor != nullptr) ? lpszVendor : L"Unknown";
...
#define SAFE_LOG_PTR  if (NULL != pLogUtil)  ((ISpLog* )pLogUtil)
	...
	strClassName = g_SpatClassContainer.Find(lpClassName);
...
CRITICAL_SECTION& CActionApp::GetLock(int nTaskId)
{
	{
    	std::lock_guard<std::mutex> lock(m_SortLock);
		m_arrSortSource.push_back(nTaskId);
        uint32 waitTime = (nTaskId - 1) * 1000 > 3000 ? 3000 : (nTaskId - 1) * 1000;
        Timer timer;
        timer.setTimeout([this]() {
            std::lock_guard<std::mutex> lock(m_SortLock);
			for (int i = 0; i < m_arrSortSource.size(); i++)
			{
				m_arrSortDest.push_back(m_arrSortSource[i]);
			}
			m_arrSortSource.clear();
			std::sort(m_arrSortDest.begin(), m_arrSortDest.end());
			}, waitTime);
	}
	...
	while (true)
	{
        std::lock_guard<std::mutex> lock(m_SortLock);
		/*std::sort(m_arrSortDest.begin(), m_arrSortDest.end());*/
        if (m_arrSortDest.size() != 0)
		{
			if (m_arrSortDest[0] == nTaskId)
			{
				//m_arrSortDest.pop_front();
				return m_Lock;
			}
		}
	}
...
void CActionApp::ReleaseLock(int nTaskId)
{
    std::lock_guard<std::mutex> lock(m_SortLock);

	//查找对应的nTaskId，并从队列中删除
    auto it = std::find(m_arrSortDest.begin(), m_arrSortDest.end(), nTaskId);
    if (it != m_arrSortDest.end()) {
        m_arrSortDest.erase(it);
    }
}
