#include "StdAfx.h"
#include "ImportGoldenSampleV4.h"
...
IMPLEMENT_RUNTIME_CLASS(CImportGoldenSampleV4)
CImportGoldenSampleV4::CImportGoldenSampleV4(void)
{
}
...
SPRESULT CImportGoldenSampleV4::ImportLTE(LPCWSTR lpPath)
{
    WCHAR szTmp[1024] = { 0 };
    WCHAR szkey[64] = { 0 };
    ...
    for (int i = 0; i < nCount; i++)
    {
        ...
        if (NULL != wcsstr(szTmp, L"TxPower"))
        {
            ...
            stData.TxData.Head.byChannelId = (uint8)nChannelId;
        }
        else
        {
            ...
            stData.RxData.Head.byChannelId = (uint8)nChannelId;
        }
...
SPRESULT CImportGoldenSampleV4::LteSerializationV4(std::vector<uint8>& arrInfo)
{    //m_gs.lte to arrInfo
    ...
    for (int i = 0; i < (int)m_vecLteData.size(); i++)   //m_vecLteData从文件里读出的数据
    {
        ...
        {
        case 1://TX
            nTempLen = sizeof(LTE_Loss_Data_TX_V4);
            CopyMemory(pData, &m_vecLteData[i].TxData, nTempLen);
            pData += nTempLen;
            break;
        case 2://RX
            nTempLen = sizeof(LTE_Loss_Data_RX_V4);
...
void CImportGoldenSampleV4::GetLossFmtStrLTE(std::wstring& strFmt, BOOL bExp, BOOL bTX)
{
    if (bTX)
    {
        if (bExp)
