...
SPRESULT CNrApiV5::RunTxPhoneCommand(BOOL bOn)
{
    if (bOn)
    {
        ...
        m_PhoneReq.Config.multion_nsa = 0;
        m_PhoneReq.Config.multion_ca = 0;

        m_PhoneReq.TX.PathGroupId = m_pLossData->PathGroup;
        m_PhoneReq.TX.PathId = m_pLossData->nChannel;
    ...
    CHKRESULT(SP_NR_CAL_ModemV5_SetTrx(m_hDUT, &m_PhoneReq));
    

    return SP_OK;
}

SPRESULT CNrApiV5::RunRxPhoneCommand()
{
    m_PhoneReq.Config.band = CNrUtility::m_BandInfo[m_pLossData->nBand].BandIdent;
    ...
    m_PhoneReq.RX.PathGroupId = m_pLossData->PathGroup;
    m_PhoneReq.RX.PathId = m_pLossData->nChannel;
    ...
    Setting.Pdet = FALSE;
    Setting.Rssi = TRUE;
    NR_CAL_TRX_RLT_RLT_V5_T Info;

    CHKRESULT(SP_NR_CAL_ModemV5_GetTrxInfo(m_hDUT, &Setting, &Info));
