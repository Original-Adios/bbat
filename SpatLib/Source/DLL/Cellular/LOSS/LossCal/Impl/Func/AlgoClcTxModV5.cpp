...
CAlgoClcTxModV5::CAlgoClcTxModV5( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CAlgoBase( lpName, pFuncCenter )
    , m_uMaxRetryTime( 30 )
{
	m_pNal = NULL;
	m_pFileConfig = NULL;
...
CAlgoClcTxModV5::~CAlgoClcTxModV5( void )
{

}

void CAlgoClcTxModV5::__SetAntCa()
{
	__super::__SetAntCa();
...
SPRESULT CAlgoClcTxModV5::PreInit()
{
...
SPRESULT CAlgoClcTxModV5::InitData()
{
    ...
    for ( uint32 i = 0; i < m_parrConfig->size(); i++ )
    {
        ...
        if ( pConfig->byPath == TX )
        {
            ...
            pSectionData->dEnp = 0;
            pSectionData->usApt = pConfig->Tx.nVoltage;
...
SPRESULT CAlgoClcTxModV5::GetResultStatic( void* pCaller
                                      , BOOL* ret
                                      , uint32 uStartBand
                                      ...
                                      , uint32 uRetryTime )
{
    CAlgoClcTxModV5* pObject = ( CAlgoClcTxModV5* )pCaller;
...
SPRESULT CAlgoClcTxModV5::GetResult( BOOL* ret
                                , uint32 /*uStartBand*/
                                , uint32 /*uStartChannel*/
                                , uint32 /*uStartSection*/
                                , uint32 /*uStopBand*/
                                ...
                                , uint32 uRetryTime )
{
    ...
    *ret = TRUE;
    ...
    IsValidIndex(m_Ant)
	pResult->usAnt = m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Tx[m_UiAnt];
    ...
    else
    {
        ...
        {
            UiSendMsg("LTE TX Loss Max Retry Time"
                , LEVEL_ITEM
                , dMinTarget
                , dPower
                , dMaxTarget
                ...
                , pChannelData->usArfcn
                , "dBm"
                , "ANT = %s; RfAnt = %d; Path = %d; Freq = %.1lf; Word = %d; RetryTime=%d"
                , g_lpAntNameAV4[m_Ant]
                , m_pNv->m_pNvV5->m_arrDownload_AntMap[pConfig->byBand].Tx[0]
                , pConfig->nPathId
                , CLteUtility::GetFreq(pChannelData->usArfcn)
...
SPRESULT CAlgoClcTxModV5::SetLoss( double dLoss )
{
	int iTx0 = m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Tx[0];
	int iTx1 = m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Tx[1];
...
SPRESULT CAlgoClcTxModV5::Check()
{
    if ( m_arrarrBandData.size() == 0 )
    {
        return SP_OK;
