...
CAlgoClcRxModV3::CAlgoClcRxModV3( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CAlgoBase( lpName, pFuncCenter )
    , m_uMaxRetryTime( 30 )
{
	m_pNal = NULL;
	m_pFileConfig = NULL;
...
CAlgoClcRxModV3::~CAlgoClcRxModV3( void )
{


}

void CAlgoClcRxModV3::__SetAntCa()
{
	__super::__SetAntCa();
...
SPRESULT CAlgoClcRxModV3::PreInit()
{
...
SPRESULT CAlgoClcRxModV3::Run()
{
	...
	m_pApi->m_TxIns = m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Tx[0];
...
SPRESULT CAlgoClcRxModV3::InitData()
{
    ...
    for ( uint32 i = 0; i < m_parrConfig->size(); i++ )
    {
        ...
        if ( pConfig->byPath == RX )
        {
            ...
            if (CLteUtility::m_BandInfo[data.nBand].UlChannel.dFreqOffset == 0)
            {
                for (int index = 0; index < MAX_NV_BAND_NUMBER_MV3; index++)
                {
                    if (m_pNv->m_pNvV3->m_BandSupportInfo[index].band_num ==
                        CLteUtility::m_BandInfo[data.nBand].BandIdent)
                    {
                        data.nTriggerArfcn = m_pNv->m_pNvV3->m_BandSupportInfo[index].ulTriggerArfcn;
                        break;
                    }
                }
            }
...
SPRESULT CAlgoClcRxModV3::CheckLoss( uint32 uIndex )
{
...
SPRESULT CAlgoClcRxModV3::AdjustLoss( BOOL& ret, uint32 uIndex )
{
    LogFmtStrA( SPLOGLV_INFO, "%s: Retry Time = %d", __FUNCTION__, m_uRetryTime );
	...
	pResult->usAnt = m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Rx[m_UiAnt];
...
SPRESULT CAlgoClcRxModV3::SetLoss( double dLoss )
{
	int iRx0 = m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Rx[0];
	int iRx1 = m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Rx[1];
    ...
    m_Loss.lteLoss.arrPoint[0].dLoss[iRx1][RF_IO_TX] = dLoss;
    m_Loss.lteLoss.arrPoint[0].dLoss[iRx1][RF_IO_RX] = dLoss;
...
SPRESULT CAlgoClcRxModV3::Check()
{
    if ( m_arrarrBandData.size() == 0 )
    {
        return SP_OK;
