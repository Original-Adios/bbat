...
CAlgoGsRxModV3::CAlgoGsRxModV3( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CAlgoBase(lpName, pFuncCenter)
, m_uMaxRetryTime(30)
, m_nMinIndex(14)
, m_nMaxIndex(108)
{
...
CAlgoGsRxModV3::~CAlgoGsRxModV3(void)
{


}

void CAlgoGsRxModV3::__SetAntCa()
{
	__super::__SetAntCa();
...
SPRESULT CAlgoGsRxModV3::PreInit()
{
...
SPRESULT CAlgoGsRxModV3::InitData()
{
    ...
    for (uint32 i = 0; i < m_parrConfig->size(); i++)
    {
		...
		if( (m_pNv->m_pNvV3->m_arrDownload_AntMap[pConfig->byBand].Tx[0] != m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Tx[0]) 
			||(m_pNv->m_pNvV3->m_arrDownload_AntMap[pConfig->byBand].Rx[0] != m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Rx[0]) 
			||(m_pNv->m_pNvV3->m_arrDownload_AntMap[pConfig->byBand].Rx[1] != m_pNv->m_pNvV3->m_arrGS_PortComp[m_ePortComp].Rx[1]) )
		{
			continue;
		}
        ...
        if (pConfig->byPath == RX)
        {
            ...
            BandData.nTriggerArfcn = pConfig->usTriggerArfcn;
...
SPRESULT CAlgoGsRxModV3::AdjustCellPower(BOOL &ret)
{
