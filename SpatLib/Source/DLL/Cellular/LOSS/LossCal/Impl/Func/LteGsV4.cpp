...
CLteGsV4::CLteGsV4(CImpBase *pSpat)
: ILossFunc(pSpat)
, m_pFuncCenter(NULL)
...
SPRESULT CLteGsV4::Init()
{
	...
	if (m_pNal == NULL)
	{
		m_pNal = new CNalGsV4(FUNC_INAL, m_pFuncCenter);
	}
	...
	if (m_pTxAlgo == NULL)
	{
		m_pTxAlgo = new CAlgoGsTxModV4(ALGO_MODEM_V4_APC, m_pFuncCenter);
	}
	
	if (m_pRxAlgo == NULL)
	{
		m_pRxAlgo = new CAlgoGsRxModV4(ALGO_MODEM_V4_AGC, m_pFuncCenter);
	}
...
void CLteGsV4::Serialization( std::vector<uint8>* parrData )
{
