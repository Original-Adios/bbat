...
CLteClcV5::CLteClcV5(CImpBase *pSpat)
: ILossFunc(pSpat)
, m_pFuncCenter(NULL)
...
CLteClcV5::~CLteClcV5(void)
{
}

SPRESULT CLteClcV5::Run()
{
	...
	CHKRESULT(m_pNvHelper->ReadBandList());
...
SPRESULT CLteClcV5::Init()
{
    ...
    if (m_pNal == NULL)
    {
        m_pNal = new CNalClcV5(FUNC_INAL, m_pFuncCenter);
    }
	...
	if (m_pTxAlgo == NULL)
	{
		m_pTxAlgo = new CAlgoClcTxModV5(ALGO_MODEM_V4_APC, m_pFuncCenter);
	}
	if (m_pRxAlgo == NULL)
	{
		m_pRxAlgo = new CAlgoClcRxModV5(ALGO_MODEM_V4_AGC, m_pFuncCenter);
	}
	if (m_pTxApi == NULL)
	{
...
void CLteClcV5::Deserialization( std::vector<uint8>* parrData )
{
