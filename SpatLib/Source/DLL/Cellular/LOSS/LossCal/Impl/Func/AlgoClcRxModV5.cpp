...
CAlgoClcRxModV5::CAlgoClcRxModV5( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CAlgoBase( lpName, pFuncCenter )
    , m_uMaxRetryTime( 30 )
{
	m_pNal = NULL;
	m_pFileConfig = NULL;
...
CAlgoClcRxModV5::~CAlgoClcRxModV5( void )
{


}

void CAlgoClcRxModV5::__SetAntCa()
{
	__super::__SetAntCa();
...
SPRESULT CAlgoClcRxModV5::PreInit()
{
    CHKRESULT( __super::PreInit() );
    
	m_pNv = (CNvHelperV5*)GetFunc(FUNC_INVHelper);
    m_pNal = ( CNalClcV5* )GetFunc( FUNC_INAL );
...
SPRESULT CAlgoClcRxModV5::Run()
{
    ...
    for ( uint32 i = 0; i < m_arrarrBandData.size(); i++ )
    {
		...
		for ( ; j < m_parrConfig->size(); j++ )
		{
			...
			if( (m_pNv->m_pNvV5->m_arrDownload_AntMap[nBand].Tx[0] != m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Tx[0])
				||(m_pNv->m_pNvV5->m_arrDownload_AntMap[nBand].Rx[0] != m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[0])
				||(m_pNv->m_pNvV5->m_arrDownload_AntMap[nBand].Rx[1] != m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[1])
                || (m_pNv->m_pNvV5->m_arrDownload_AntMap[nBand].Rx[2] != m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[2])
                || (m_pNv->m_pNvV5->m_arrDownload_AntMap[nBand].Rx[3] != m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[3]))
			{
				continue;
			}
...
SPRESULT CAlgoClcRxModV5::InitData()
{
    if (m_ePortComp >= m_pNv->m_pNvV5->m_arrGS_PortComp.size())
    {
      return SP_OK;
    }
    ...
    for ( uint32 i = 0; i < m_parrConfig->size(); i++ )
    {
        ...
        if ( pConfig->byPath == RX )
        {
			...
			data.nPathGroupIndex = pConfig->nPathGroupIndex;
			data.nPathId = pConfig->nPathId;
			data.nTriggerUpLinkStream = pConfig->nTriggerUpLinkStream;
			data.nTriggerPathId = pConfig->nTriggerPathId;
			data.nTriggerGroupIndex = pConfig->nTriggerPathGroupIndex;
...
SPRESULT CAlgoClcRxModV5::CheckLoss( uint32 uIndex )
{
...
SPRESULT CAlgoClcRxModV5::AdjustLoss( BOOL& ret, uint32 uIndex )
{
    LogFmtStrA( SPLOGLV_INFO, "%s: Retry Time = %d", __FUNCTION__, m_uRetryTime );
    ...
    IsValidIndex(m_Ant)
	pResult->usAnt = m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[m_UiAnt];
    ...
    if ( IN_RANGE( dMinTarget, dRssi, dMaxTarget ) )
    {
        ...
        UiSendMsg( "LTE RX Loss"
                   ...
                   , pChannelData->usArfcn
                   , "dBm"
                   , "ANT = %s; RfAnt = %d; Path = %d; Freq = %.1lf; Index = %d;CellPower = %.1lf; Rssi = %.1lf"
                   , g_lpAntNameAV4[m_Ant]
    ...
    else
    {
        if ( m_uRetryTime == m_uMaxRetryTime )
        {
            UiSendMsg( "LTE RX Rssi"
                       ...
                       , pChannelData->usArfcn
                       , "dBm"
                       , "ANT = %s;RfAnt = %d; Path = %d; Index = %d; Freq = %.1lf"
                       , g_lpAntNameAV4[m_Ant]
                        , pConfig->nPathId
                        , m_pNv->m_pNvV5->m_arrDownload_AntMap[pConfig->byBand].Rx[m_Ant]
				        , pPointData->usGainIndex
...
SPRESULT CAlgoClcRxModV5::SetLoss( double dLoss )
{
    IsValidIndex(m_UiAnt)
	int iRx0 = m_pNv->m_pNvV5->m_arrGS_PortComp[m_ePortComp].Rx[m_UiAnt];
...
SPRESULT CAlgoClcRxModV5::Check()
{
    if ( m_arrarrBandData.size() == 0 )
    {
        return SP_OK;
