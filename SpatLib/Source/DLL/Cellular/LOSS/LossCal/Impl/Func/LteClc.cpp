...
CLteClc::CLteClc(CImpBase *pSpat)
: ILossFunc(pSpat)
, m_pFuncCenter(NULL)
...
CLteClc::~CLteClc(void)
{
}

SPRESULT CLteClc::Run()
{
	...
	else if(m_pImp->m_eModemVer == MV_V3)
	{
		...
		CHKRESULT(m_pNvHelper->m_pNvV3->LoadSupportBandInfo());
		CHKRESULT(m_pSpatBase->m_pRFTester->SetNetMode(NM_LTE));
		CHKRESULT(m_pSpatBase->m_pRFTester->InitDev(TM_NON_SIGNAL, SUB_CAL_TRX_FDT, NULL));
...
SPRESULT CLteClc::Init()
{
    ...
    if (m_pNal == NULL)
    {
        m_pNal = new CNalClc(FUNC_INAL, m_pFuncCenter);
    }
	...
	else if(m_pImp->m_eModemVer == MV_V3)
	{
		...
		if (m_pTxAlgoMainPcc == NULL)
		{
			m_pTxAlgoMainPcc = new CAlgoClcTxModV3(ALGO_APC_MAIN_PCC, m_pFuncCenter);
		}
		if (m_pTxAlgoDivScc == NULL)
		{
			m_pTxAlgoDivScc = new CAlgoClcTxModV3(ALGO_APC_DIV_SCC, m_pFuncCenter);
		}
		if (m_pRxAlgoMainPcc == NULL)
		{
			m_pRxAlgoMainPcc = new CAlgoClcRxModV3(ALGO_AGC_MAIN_PCC, m_pFuncCenter);
		}
		if (m_pRxAlgoDivPcc == NULL)
		{
			m_pRxAlgoDivPcc = new CAlgoClcRxModV3(ALGO_AGC_DIV_PCC, m_pFuncCenter);
		}
		if (m_pTxApi == NULL)
		{
...
void CLteClc::Deserialization( std::vector<uint8>* parrData )
{
