...
CApiTxModV4::CApiTxModV4( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CApiBase(lpName, pFuncCenter)
{
...
SPRESULT CApiTxModV4::PreInit()
{
    ...
    if (m_pPhoneParamV4 == NULL)
    {
        m_pPhoneParamV4 = new PC_MODEM_RF_V4_LTE_APC_REQ_CMD_T();
    }
...
SPRESULT CApiTxModV4::GetMaxPointsNumber()
{
    return SP_OK;
}

SPRESULT CApiTxModV4::InsertSection( BOOL* ret, int nBand, int nChannel, int nSection, BOOL bGap )
{
    ...
    int nChannelIndex = m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl->ArfcnNum - 1;
    ...
    m_pPhoneParamV4->nAptNum[m_RfChain]++;

	if(PhoneApt.PdetFlag)
	{
        m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].PdetNum += PhoneApt.StepNum*3;
	}
...
SPRESULT CApiTxModV4::InsertChannel( BOOL* ret, int nBand, int nChannel, int nSection )
{
    BandData* pBandData = &(*m_parrBandData)[nBand];
    ChannelData* pChannelData = &pBandData->arrChannel[nChannel];
    SectionData* pSectionData = &pChannelData->arrSection[nSection];
    ...
    if (m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[m_RfChain].ArfcnNum == 0)
    {
    ...
    if (m_uTxSize + sizeof(CALI_APC_ARFCN_V4_T) + sizeof(CALI_APC_APT_CONFIG_T) > m_uMaxSize)
    {
        *ret = FALSE;
        return SP_OK;
    }
    ...
    //phone
    CALI_APC_ARFCN_V4_T PhoneChannel;
	ZeroMemory(&PhoneChannel, sizeof(CALI_APC_ARFCN_V4_T));
    ...
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].ArfcnNum++;
    m_uTxSize += sizeof(CALI_APC_ARFCN_V4_T);

    PhoneChannel.RF_CH = (uint16)pBandData->nChannelId;
...
SPRESULT CApiTxModV4::DoFunc()
{
	...
	BOOL bPdet = (0 != m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].PdetNum);
	if(bPdet)
	{
		m_vecPdetRslt.resize(m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].PdetNum);
    ...
    m_pPhoneParamV4->pArfcnConfChian0 = &m_arrPhoneChannel[0];
    m_pPhoneParamV4->pAptConfChain0 = &m_arrPhoneApt[0];
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].AptOffset = 4 + sizeof(CALI_APC_CTRL_HEADER_T)
		+ m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].ArfcnNum*sizeof(CALI_APC_ARFCN_V4_T);
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[0].ArfcnOffset = 4 + sizeof(CALI_APC_CTRL_HEADER_T);
    ...
    Result = SP_ModemV4_LTE_ApcCal(m_hDUT, m_pPhoneParamV4, bPdet, m_pPhoneRet, nPdetRlstSize);
...
void CApiTxModV4::InitParam( int nBand, int nChannel, int nSection )
{
    ZeroMemory(m_pPhoneParamV4, sizeof(PC_MODEM_RF_V4_LTE_APC_REQ_CMD_T));
	...
	//V3
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[m_RfChain].BW = LTE_BW_5M;
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[m_RfChain].Ant = (uint8)m_Ant;
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[m_RfChain].RfChain = (uint8)m_RfChain;

    m_pPhoneParamV4->stReqHeader.HeaderSize = sizeof(CALI_APC_CTRL_HEADER_T);
    m_pPhoneParamV4->stReqHeader.AptSize = sizeof(CALI_APC_APT_CONFIG_T);
    m_pPhoneParamV4->stReqHeader.ArfcnSize = sizeof(CALI_APC_ARFCN_V4_T);
    m_pPhoneParamV4->stReqHeader.CaFlag = NON_CA;
...
SPRESULT CApiTxModV4::InsertTrigger( int nBand, int nChannel )
{
    LogFmtStrA(SPLOGLV_INFO, "%s", __FUNCTION__);
    ...
    m_arrPhoneChannel.push_back(PhoneChannel);
    m_pPhoneParamV4->stReqHeader.ApcRfChainCtrl[m_RfChain].ArfcnNum++;
