...
CApiRxModV3::CApiRxModV3( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CApiBase(lpName, pFuncCenter)
{
...
SPRESULT CApiRxModV3::PreInit()
{
	...
	try
	{
		if (m_pPhoneParam == NULL)
		{
			m_pPhoneParam = new PC_MODEM_RF_V3_LTE_AGC_PARAM();
		}
...
SPRESULT CApiRxModV3::DoFunc()
{
	if (m_pPhoneParam->Header.AgcRfChain[0].RxGainNum == 0)
	{
		return SP_OK;
	}
	...
	if (m_ApiRetRssi.size() < m_pPhoneParam->Header.AgcRfChain[0].RxGainNum)
	{
		m_ApiRetRssi.resize(m_pPhoneParam->Header.AgcRfChain[0].RxGainNum);
	}
	//Instrument
	RF_LTE_PARAM param;
	...
	//DUT
	m_pPhoneParam->ChannelsChian0 = &m_vecApiChannels[0];
	m_pPhoneParam->PointsChain0 = &m_ApiPoints[0];
	m_pPhoneParam->Header.AgcRfChain[0].RxGainOffset = 4 
		+  sizeof(PC_MODEM_RF_V3_LTE_AGC_HEADER_T)
		+ sizeof(PC_MODEM_RF_V3_LTE_AGC_TRIGGER_T)
		+ m_pPhoneParam->Header.AgcRfChain[0].ArfcnNum*sizeof(PC_MODEM_RF_V3_LTE_AGC_CHANNEL_T);
	...
	for(uint32 i = 0; i <= m_uMaxRetryCout; i++)
	{
		...
		Result = SP_ModemV3_LTE_AgcCal( m_hDUT, m_pPhoneParam, (uint16*)&m_ApiRetRssi[0]);
		if (Result != SP_OK)
		{
...
SPRESULT CApiRxModV3::InsertGainIndex( int nBand, int nChannel, int nGain )
{
    int i = nBand;
    int j = nChannel;
    int k = nGain;
    ...
    int nChannelIndex = m_pPhoneParam->Header.AgcRfChain[0].ArfcnNum - 1;
    int nPointIndex = m_pPhoneParam->Header.AgcRfChain[0].RxGainNum;
    //Dut
    int nSize = m_ApiPoints.size();
    if (nSize == m_pPhoneParam->Header.AgcRfChain[0].RxGainNum)
    {
        nSize += 100;
    ...
    m_pPhoneParam->Header.AgcRfChain[0].RxGainNum++;
...
SPRESULT CApiRxModV3::InsertChannel( int nBand, int nChannel )
{
	...
	if (nSize == m_pPhoneParam->Header.AgcRfChain[0].ArfcnNum)
	{
		nSize += 100;
		m_vecApiChannels.resize(nSize);
	}

	m_pPhoneParam->Header.AgcRfChain[0].ArfcnNum++;
	int nChannelIndex = m_pPhoneParam->Header.AgcRfChain[0].ArfcnNum - 1;

	PC_MODEM_RF_V3_LTE_AGC_CHANNEL_T Channel;
	ZeroMemory(&Channel, sizeof(PC_MODEM_RF_V3_LTE_AGC_CHANNEL_T));
	...
	if (1 == m_pTesterParam->nChannelCount )
	{
		...
		if (pBandData->nTriggerArfcn != 0)
		{
			m_pPhoneParam->Trigger.Arfcn = pBandData->nTriggerArfcn;
			m_pTesterParam->usTriggerChannel = pBandData->nTriggerArfcn;
			if (m_bBandAdaptation)
			{
				LTE_BAND_E nband = CLteUtility::GetBand(m_pPhoneParam->Trigger.Arfcn);
				IsValidIndex(nband);
				m_pPhoneParam->Trigger.Band = CLteUtility::m_BandInfo[nband].BandIdent;
			}
		}
		else
		{
			...
			if(m_bBandAdaptation)
			{
				m_pPhoneParam->Trigger.Band = CLteUtility::m_BandInfo[pBandData->nBand].BandIdent;
			}
...
void CApiRxModV3::InitParam()
{
    ZeroMemory(m_pPhoneParam,sizeof(PC_MODEM_RF_V3_LTE_AGC_PARAM));
	...
	//////////////////////////////////////////////////////////////////////////
	//Modem V3
	m_pPhoneParam->Header.HeaderSize = sizeof(PC_MODEM_RF_V3_LTE_AGC_HEADER_T);
	m_pPhoneParam->Header.ArfcnSize = sizeof(PC_MODEM_RF_V3_LTE_AGC_CHANNEL_T);
	m_pPhoneParam->Header.TrigSize = sizeof(PC_MODEM_RF_V3_LTE_AGC_TRIGGER_T);
	m_pPhoneParam->Header.RxGainSize = sizeof(PC_MODEM_RF_V3_LTE_AGC_GAIN_T);
	m_pPhoneParam->Header.CaFlag = NON_CA;
	//Rf chain config
	m_pPhoneParam->Header.AgcRfChain[0].Ant = (uint8)(m_Ant % 2);
	m_pPhoneParam->Header.AgcRfChain[0].RfChain = (uint8)m_RfChain;

	if(!m_bVsBW_Enable)
	{
		m_pPhoneParam->Header.AgcRfChain[0].Bw = LTE_BW_5M;
	}
	else
	{
		if(LTE_RF_BW0 == m_Bw)
		{
			m_pPhoneParam->Header.AgcRfChain[0].Bw = LTE_BW_3M;
		}
		else
		{
			m_pPhoneParam->Header.AgcRfChain[0].Bw = LTE_BW_5M;
	...
	m_pPhoneParam->Header.AgcRfChain[0].ArfcnOffset = 4 +  sizeof(PC_MODEM_RF_V3_LTE_AGC_HEADER_T)+ sizeof(PC_MODEM_RF_V3_LTE_AGC_TRIGGER_T);
	m_pPhoneParam->Header.AgcRfChain[0].TrigOffset = 4 +  sizeof(PC_MODEM_RF_V3_LTE_AGC_HEADER_T);
	m_pPhoneParam->Header.NopTime = 1;
	//Trigger config
	m_pPhoneParam->Trigger.Ant = (uint8)(m_Ant % 2);
	m_pPhoneParam->Trigger.Apt = 35;
	m_pPhoneParam->Trigger.PaMode = 0;
	m_pPhoneParam->Trigger.RfChain = (uint8)0;
...
SPRESULT CApiRxModV3::Init()
{
    return TRUE;
}

void CApiRxModV3::ClearData()
{
