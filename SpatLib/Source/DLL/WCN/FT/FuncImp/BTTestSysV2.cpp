#include "StdAfx.h"
#include <cassert>
#include "BTTestSysV2.h"
#include "BTLoadXML.h"
#include "SimpleAop.h"
IMPLEMENT_RUNTIME_CLASS(CBTTestSysV2)

CBTTestSysV2::CBTTestSysV2(void)
: m_pEnterMode(NULL)
, m_pBtApi(NULL)
...
SPRESULT CBTTestSysV2::__InitAction(void)
{
	...
	m_pBleExMeas = new CBTMeasBLEEx(this, m_pBtApi);
	if(NULL == m_pBleExMeas)
	{
...
BOOL CBTTestSysV2::LoadXMLConfig(void)
{
	auto _function = SimpleAop(this, __FUNCTION__);
    ...
    CBTLoadXML *pLoadXml = new CBTLoadXML(this);
...
SPRESULT CBTTestSysV2::__PollAction( void )
{
	...
	for (int nBIndx=0; nBIndx<(int)m_VecBTParamBandImp.stBTParamBand.size(); nBIndx++)
	{
		...
		case BLE_EX:
		{
			...
			if (ANT_SINGLE == eRfPath && nRfSwitchAnt1st > 0)
			{
				m_pBleExMeas->m_Customize.SetRfSwitch(eMode, nRfSwitchAnt1st, nRfSwitchAnt1st);
			}

			if (ANT_SHARED == eRfPath && nRfSwitchAnt2nd > 0)
			{
				m_pBleExMeas->m_Customize.SetRfSwitch(eMode, nRfSwitchAnt2nd, nRfSwitchAnt2nd);
			}
			m_pBleExMeas->BTParamSet(&m_VecBTParamBandImp.stBTParamBand[nBIndx]);
			eRes = m_pBleExMeas->BTMeasPerform();
			//Customer control RF Port
			m_pBleExMeas->m_Customize.ResetRfSwitch();
		}
			break;
		default:
			break;
...
void CBTTestSysV2::__LeaveAction( void )
{
}

SPRESULT CBTTestSysV2::__FinalAction(void)
{
	auto _function = SimpleAop(this, __FUNCTION__);
    if (NULL != m_pEnterMode)
    {
