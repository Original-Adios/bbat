...
CBTLoadXML::CBTLoadXML(CSpatBase *pSpatBase)
: m_pSpatBase(pSpatBase)
{
}

CBTLoadXML::~CBTLoadXML(void)
{
    if (NULL != m_pSpatBase)
    {
        m_pSpatBase = NULL;
...
SPRESULT CBTLoadXML::BTLoadXmlFile(vector<BTMeasParamBand> &VecBTParamBandImp)
{
	...
	for(int nAnt=ANT_SINGLE; nAnt<ANT_BT_MAX; nAnt++ )
	{
		...
		for(int nMode=BDR; nMode<MAX_BT_TYPE; nMode++)
		{
			 ...
			 for(int nGroupIndex=0; nGroupIndex<nGroupCount; nGroupIndex++)
			 {
				 ...
				 //MAX ICFT
				 bEnable = (BOOL)m_pSpatBase->GetConfigValue((strMask + L"ICFTMAX").c_str(), FALSE);
				 if (bEnable)
				 {
					 dwMask |= FREQ_MAXICFR;
				 }
				 //MIN ICFT
				 bEnable = (BOOL)m_pSpatBase->GetConfigValue((strMask + L"ICFTMIN").c_str(), FALSE);
				 if (bEnable)
				 {
					 dwMask |= FREQ_MINICFR;
				 }
				 ...
				 //MAX CFD
				 bEnable = (BOOL)m_pSpatBase->GetConfigValue((strMask + L"CFDMAX").c_str(), FALSE);
				 if (bEnable)
				 {
					 dwMask |= FREQ_MAXCFD;
				 }
				 //MIN CFD
				 bEnable = (BOOL)m_pSpatBase->GetConfigValue((strMask + L"CFDMIN").c_str(), FALSE);
				 if (bEnable)
				 {
					 dwMask |= FREQ_MINCFD;
				 }
				 ...
				 bEnable = (BOOL)m_pSpatBase->GetConfigValue((strMask + L"RSSI").c_str(), FALSE);
				 if (bEnable)
				 {
					 dwMask |= BLE_RSSI;
				 }
			 ...
			 if (nMode == BDR)
			 {
				 ...
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":ICFT").c_str(), L"-75,75");
				 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if(nCount < 2)
				 {
				 ...
				 //MAX ICFT
				 //ICFT Initial carrier frequency tolerance
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":ICFTMAX").c_str(), L"-45,45");
				 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if (nCount < 2)
				 {
					 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MAX BDR initial carrier frequency tolerance limit");
					 return SP_E_WCN_INVALID_XML_CONFIG;
				 }
				 stBTParamBand.stSpec.stBdrSpec.dIcftMax.low = parrInt[0];
				 stBTParamBand.stSpec.stBdrSpec.dIcftMax.upp = parrInt[1];
				 //MIN ICFT
				 //ICFT Initial carrier frequency tolerance
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":ICFTMIN").c_str(), L"-45,45");
				 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if (nCount < 2)
				 {
					 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MIN BDR initial carrier frequency tolerance limit");
					 return SP_E_WCN_INVALID_XML_CONFIG;
				 }
				 stBTParamBand.stSpec.stBdrSpec.dIcftMin.low = parrInt[0];
				 stBTParamBand.stSpec.stBdrSpec.dIcftMin.upp = parrInt[1];
				 ...
				 //MAX Carrier Frequency Drift
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFDMAX").c_str(), L"-20,20");
				 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if (nCount < 2)
				 {
					 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MAX BDR carrier frequency drift limit");
					 return SP_E_WCN_INVALID_XML_CONFIG;
				 }
				 stBTParamBand.stSpec.stBdrSpec.dCfdMax.low = parrInt[0];
				 stBTParamBand.stSpec.stBdrSpec.dCfdMax.upp = parrInt[1];

				 //MIN Carrier Frequency Drift
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFDMIN").c_str(), L"-20,20");
				 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if (nCount < 2)
				 {
					 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MIN BDR carrier frequency drift limit");
					 return SP_E_WCN_INVALID_XML_CONFIG;
				 }
				 stBTParamBand.stSpec.stBdrSpec.dCfdMin.low = parrInt[0];
				 stBTParamBand.stSpec.stBdrSpec.dCfdMin.upp = parrInt[1];

				 //BER spec
				 stBTParamBand.stSpec.stBdrSpec.dBer = m_pSpatBase->GetConfigValue((strSpec + L":BER").c_str(), 0.1);
				 //RSSI
				 strVal = m_pSpatBase->GetConfigValue((strSpec + L":RSSI").c_str(), L"-5,5");
				 parrDouble = m_pSpatBase->GetTokenDoubleW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
				 if (nCount < 2)
				 {
					 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid RSSI limit");
					 return SP_E_WCN_INVALID_XML_CONFIG;
				 }
			 ...
			 else if(nMode == EDR)
			 {
				 ...
				 stBTParamBand.stSpec.stEdrSpec.dP99Evm_2DHX.upp = m_pSpatBase->GetConfigValue((strSpec + L":CFDMODA:P99DEVM_2DHX").c_str(), 0.30);
				 ...
				 stBTParamBand.stSpec.stEdrSpec.dP99Evm_3DHX.upp = m_pSpatBase->GetConfigValue((strSpec + L":CFDMODA:P99DEVM_3DHX").c_str(), 0.25);
			 ...
			 else
			 {
				 ...
				 for(int nPacket=RF_PHY_1M; nPacket<RF_PHY_MAX; nPacket++)
				 {
					 ...
					 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFOD:"+ lpName + lpItem).c_str(), L"-19.2,19.2");
					 parrDouble = m_pSpatBase->GetTokenDoubleW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
					 if(nCount < 2)
					 {
					 ...
					 //MAX ICFT
					 //ICFT Initial carrier frequency tolerance
					 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFOD:" + lpName + L":ICFTMAX").c_str(), L"-45,45");
					 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
					 if (nCount < 2)
					 {
						 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MAX BDR initial carrier frequency tolerance limit");
					 ...
					 stBTParamBand.stSpec.stBleExSpec.dIcftMax[nPacket].low = parrInt[0];
					 stBTParamBand.stSpec.stBleExSpec.dIcftMax[nPacket].upp = parrInt[1];
					 //MIN ICFT
					 //ICFT Initial carrier frequency tolerance
					 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFOD:" + lpName + L":ICFTMIN").c_str(), L"-45,45");
					 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
					 if (nCount < 2)
					 {
						 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MIN BDR initial carrier frequency tolerance limit");
					 ...
					 stBTParamBand.stSpec.stBleExSpec.dIcftMin[nPacket].low = parrInt[0];
					 stBTParamBand.stSpec.stBleExSpec.dIcftMin[nPacket].upp = parrInt[1];

					 //MAX Carrier Frequency Drift
					 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFOD:" + lpName + L":CFDMAX").c_str(), L"-20,20");
					 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
					 if (nCount < 2)
					 {
						 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MAX BDR carrier frequency drift limit");
					 ...
					 stBTParamBand.stSpec.stBleExSpec.dCfdMax[nPacket].low = parrInt[0];
					 stBTParamBand.stSpec.stBleExSpec.dCfdMax[nPacket].upp = parrInt[1];

					 //MIN Carrier Frequency Drift
					 strVal = m_pSpatBase->GetConfigValue((strSpec + L":CFOD:" + lpName + L":CFDMIN").c_str(), L"-20,20");
					 parrInt = m_pSpatBase->GetTokenIntegerW(strVal.c_str(), DEFAULT_DELIMITER_W, nCount);
					 if (nCount < 2)
					 {
						 m_pSpatBase->LogFmtStrA(SPLOGLV_ERROR, "[BT FT]Invalid MIN BDR carrier frequency drift limit");
					 ...
					 stBTParamBand.stSpec.stBleExSpec.dCfdMin[nPacket].low = parrInt[0];
					 stBTParamBand.stSpec.stBleExSpec.dCfdMin[nPacket].upp = parrInt[1];
				 ...
				 stBTParamBand.stSpec.stBleExSpec.dRssi.low = parrDouble[0];
				 stBTParamBand.stSpec.stBleExSpec.dRssi.upp = parrDouble[1];
