...
//////////////////////////////////////////////////////////////////////////
//
CBTMeasEDR::CBTMeasEDR(CImpBase *pImpBase, ICBTApi *pBtApi) 
: CBTMeasBase(pImpBase, pBtApi)
{

}

CBTMeasEDR::~CBTMeasEDR(void)
{
  
}

BOOL CBTMeasEDR::MeasureUplink(BTMeasParamChan *pUplinkMeasParam, SPBT_RESULT_T *pTestResult)
{
...
BOOL CBTMeasEDR::TestPER(BTMeasParamChan *pDownlinkMeasParam, SPBT_RESULT_T* pRxReult)
{
...
//	Sleep(1000);
	...
	//Rx ON
    CHKRESULT_WITH_NOTIFY_BT_RX_OFF(m_pBTApi->DUT_RFOn(BT_RX, true), "DUT_RFOn");
    Sleep(200);
    ...
    do 
    {
        if (SP_OK != m_pBTApi->DUT_GetRxData(error_bits, total_bits, error_pkts, total_pkts, rssi))
        {
			...
			//// RX OFF
            CHKRESULT_WITH_NOTIFY_BT_RX_OFF(m_pBTApi->DUT_RFOn(BT_RX, FALSE), "DUT_RFOff");
