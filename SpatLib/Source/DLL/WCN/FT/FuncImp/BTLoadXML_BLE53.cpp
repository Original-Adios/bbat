...
CBTLoadXML_BLE53::CBTLoadXML_BLE53(CSpatBase *pSpatBase)
: m_pSpatBase(pSpatBase)
{
}

CBTLoadXML_BLE53::~CBTLoadXML_BLE53(void)
{
    if (NULL != m_pSpatBase)
    {
        m_pSpatBase = NULL;
...
SPRESULT CBTLoadXML_BLE53::BTLoadXmlFile(vector<BTMeasParamBand> &VecBTParamBandImp)
{
	...
	wstring strMode[MAX_BT_TYPE] = {L"BDR",L"EDR", L"BLE", L"BLE5.0", L"BLE5.3" };
	...
	for(int nAnt=ANT_SINGLE; nAnt<ANT_BT_MAX; nAnt++ )
	{
		...
		for(int nMode=BDR; nMode<MAX_BT_TYPE; nMode++)
		{
			 ...
			 for(int nGroupIndex=0; nGroupIndex<nGroupCount; nGroupIndex++)
			 {
				 ...
				 int nNumAntID = m_pSpatBase->GetConfigValue((strGroup + L":NumAntID").c_str(), 2);
				 int nCteType = m_pSpatBase->GetConfigValue((strGroup + L":CteType").c_str(), 0);
				 int nCteLength = m_pSpatBase->GetConfigValue((strGroup + L":CteLength").c_str(), 0);
				 int nSlotDurations = m_pSpatBase->GetConfigValue((strGroup + L":SlotDurations").c_str(), 1);
				 ...
				 for (int nChTmp=0;nChTmp<(int)ChGroup.size();nChTmp++)
				 {
					 ...
					 stChanParam.nNumAntID = nNumAntID - 2;
					 stChanParam.nCteType = nCteType;
					 stChanParam.nCteLength = nCteLength;
					 stChanParam.nSlotDurations = nSlotDurations;
