...
CWlanLoadXMLOS80::CWlanLoadXMLOS80(CSpatBase *pSpatBase)
...
CWlanLoadXMLOS80::~CWlanLoadXMLOS80(void)
{
	if (NULL != m_pSpatBase)
	{
		m_pSpatBase = NULL;
...
SPRESULT CWlanLoadXMLOS80::WlanLoadXmlFile( WLAN_PARAM_CONF &stWlanParamImp )
{
	...
	for(int nBandIndex=WLAN_BAND_24G; nBandIndex<WLAN_BAND_MAX; nBandIndex++)
	{
		for(int nAnt = ANT_PRIMARY; nAnt<ANT_MAX; nAnt++)
		{
			...
			for(int nProtoc=WIFI_802_11b; nProtoc<WIFI_MaxProtocol; nProtoc++)
			{
				...
				//dLvlGap
				m_stWlanParamBand.stSpec.dLvlGap = m_pSpatBase->GetConfigValue((strPath + L":LVLGAP").c_str(), 3.0);
				//RSSI spec
...
SPRESULT CWlanLoadXMLOS80::WlanLoadLossXmlFile(WLAN_LOSS_PARAM& wlanLossParam)
{
