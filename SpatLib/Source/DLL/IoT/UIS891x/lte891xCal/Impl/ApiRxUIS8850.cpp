...
CApiRxUIS8850::CApiRxUIS8850( LPCWSTR lpName, CFuncCenter* pFuncCenter ) : CApiBase(lpName, pFuncCenter)
{
...
SPRESULT CApiRxUIS8850::PreInit()
{
...
SPRESULT CApiRxUIS8850::DoFunc()
{
	...
	for(uint32 i = 0; i <= m_uMaxRetryCout; i++)
	{
        ...
        if (Result != SP_OK)
        {
            LogRawStrA(SPLOGLV_ERROR, "FetchResult failed,adjust TriggerLevl!");
            if (i < m_uMaxRetryCout)
            {
                m_DeltaTriggerLevl = 3 * m_TriggerLevlNum;
                m_pTesterParam->nTriggerThreshold = nTriggerThreshold - m_DeltaTriggerLevl;
                m_TriggerLevlNum++;
                continue;
            }
...
SPRESULT CApiRxUIS8850::InsertGainIndex(int nBand, int nChannel, int nGain )
{
    ...
    Point.Reserved = pPointData->usLnaStatus;//set lna status
...
SPRESULT CApiRxUIS8850::InsertChannel( int nBand, int nChannel )
{
	BandData* pBandData = &(*m_parrBandData)[nBand];
	ChannelData* pChannelData = &pBandData->arrChannel[nChannel];
	...
	if (1 == m_pTesterParam->nChannelCount )
	{
		...
		if (NULL !=m_pAgcTrigger)
		{
            bool bBand58 = false;
            if ((0<= pBandData->nBand) && (pBandData->nBand< MAX_LTE_BAND))
            {
                if ((CLteUtility::m_BandInfo[pBandData->nBand].nNumeral == 58)
                    &&(CLteUtility::m_BandInfo[pBandData->nBand].nIndicator == 65))
                {
                    bBand58 = true;
                }
            }
            if (m_pAgcTrigger->bLteBand2WifiCal && bBand58)
			{
				m_pPhoneParam->Trigger.Arfcn = m_pAgcTrigger->arfcn;
				m_pPhoneParam->Trigger.Indicator = m_pAgcTrigger->indicator;
				m_pTesterParam->usTriggerChannel = m_pAgcTrigger->arfcn;
                LogFmtStrA(SPLOGLV_INFO, "%s: wlan calibration via lte,trigger by band=%d,arfcn=%d,", __FUNCTION__
                , CLteUtility::m_BandInfo[m_pAgcTrigger->nBand].nNumeral, m_pAgcTrigger->arfcn);
			}
		}
	}
...
void CApiRxUIS8850::InitParam(int nBand, int nChannel, int nGain)
{
	...
	if (0x8850 == m_UeType)
	{
		m_pPhoneParam->Header.Reserved[0] = (unsigned char)(m_RxBwConfig);
...
//        m_pPhoneParam->Trigger.Word = 69;//max power
    ...
    {
        m_pPhoneParam->Trigger.PaMode = 1;
//        m_pPhoneParam->Trigger.Word = 200;//16; //0x20;
    }
...
//        m_pTesterParam->nTriggerPower = 23;
	...
	{
//        m_pTesterParam->nTriggerPower = 5+5;//23;
...
SPRESULT CApiRxUIS8850::GetData(uint32 nRetryCount)
{
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    {   
        ...
        {       
            ...
            for (k = nTempStartGain; k <= nTempStopGain; k++)
            {
                ...
                double dRssi = 10 * log10((double)m_ApiRetRssi[nGainIndex] * 128) - 10*log10(pow((double)2, 22)) - 30;
...
SPRESULT CApiRxUIS8850::Init()
{
    return TRUE;
}

void CApiRxUIS8850::ClearData()
{
    uint32 iArrBandDataSize = m_parrBandData->size();
...
SPRESULT CApiRxUIS8850::SwitchCa()
{
...
SPRESULT CApiRxUIS8850::SetAgcParam()
{
