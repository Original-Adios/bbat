...
string_t CInputBarCodes::GetJsonFile(void)
{
    ...
    _stprintf_s(szFileName, _T("DiDi_BarCode_%d.Json"), GetAdjParam().nTASK_ID);
...
BOOL CInputBarCodes::LoadXMLConfig(void)
{
    ...
    LPCWSTR SUPPORT_LABELS[MAX_BARCODE_NUM] = {L"DeviceName", L"SN1", L"IMEI1", L"WIFI", L"BT"};
...
BOOL CInputBarCodes::LoadJsonFile(LPCTSTR lpszJsonFile)
{
    ...
    //  NOTIFY(m_lstCodes[i].szLabel, LEVEL_UI, 1, 1, 1, NULL, -1, NULL, m_lstCodes[i].szCode);
    }
...
SPRESULT CInputBarCodes::__PollAction(void)
{
    ...
    CHKRESULT(SetShareMemory(ShareMemory_DiDiInputCodes, &m_lstCodes, sizeof(BarCode_T)*MAX_BARCODE_NUM));
    return SP_OK;
}