#include "StdAfx.h"
#include "WriteSSNSN1.h"
#include "SharedDefine.h"
#include "BarCodeUtility.h"
#include "timo_crypt.h"
//#include "ExtraLogFile.h"
//#pragma comment(lib, "./Lib/cryptlib_SN1.lib")

IMPLEMENT_RUNTIME_CLASS(CWriteSSNSN1)

#define MISCDATA_FOR_SSN_LENGTH 47
//////////////////////////////////////////////////////////////////////////
CWriteSSNSN1::CWriteSSNSN1(void)
{
...
BOOL CWriteSSNSN1::LoadXMLConfig(void)
{
    return TRUE;
}

SPRESULT CWriteSSNSN1::__PollAction(void)
{ 
	...
	do
	{
		...
		NOTIFY("WriteSSNSN1", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL, "Get SN2 fail");
		return SP_E_FAIL;
...
#pragma warning(default:4127)
	...
	if(MES_SUCCESS == mesResult)
	{
		...
		if (SP_OK != sp_result)
		{
			NOTIFY("WriteSSNSN1", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL, "GetShareMemory(ShareMemory_MES_Assigned_Codes)");
			return sp_result;
		}
	}

	//生成SSN
	std::string strSSN = timo_crypt::EncryptSSNToS1(szSN2);
    char szSSN[MISCDATA_FOR_SSN_LENGTH] = {0};

	if(MISCDATA_FOR_SSN_LENGTH!=strSSN.length())
	{
		NOTIFY("WriteSSNSN1", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL, "Get SSN fail(SSN:%s,length:%d])",strSSN.c_str(),strlen(strSSN.c_str()));
		return SP_E_FAIL;
	}
	strncpy_s(szSSN, strSSN.c_str(), MISCDATA_FOR_SSN_LENGTH - 1);
	szSSN[MISCDATA_FOR_SSN_LENGTH - 1] = '\0';//舍去最后一位
	NOTIFY("WriteSSNSN1", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, "SSN:%s(length:%d)",szSSN,strlen(szSSN));
	...
	//
	NOTIFY("WriteSSNSN1", LEVEL_ITEM, 1, 1, 1, NULL, -1, NULL, "SaveCustomerMiscData");
    return SP_OK;
}
