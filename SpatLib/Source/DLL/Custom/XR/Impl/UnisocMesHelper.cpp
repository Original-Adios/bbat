...
SPRESULT CUnisocMesHelper::UnisocMesGetAssignedCodes(INPUT_CODES_T input_codes[],UINT32 uiSize)
{
	...
	if (_E_UNISOC_MES_V2 == mes_type && _E_FLOW_CONTROL_SN2 == flow_control)
	{
		sp_result = m_pImp->GetShareMemory(ShareMemory_SN2, (void*)szSN, sizeof(szSN));
		if (SP_OK != sp_result || 0 == strlen(szSN))
		{
			...
			if (0 == strlen(input_codes[BC_SN2].szCode))
			{
		...
		{
			input_codes[BC_SN2].bEnable = true;
			strcpy_s(input_codes[BC_SN2].szCode,sizeof(input_codes[BC_SN2].szCode),szSN);
		}
	}
	else
	{		
		sp_result = m_pImp->GetShareMemory(ShareMemory_SN1, (void* )szSN, sizeof(szSN));
		if (SP_OK != sp_result || 0 == strlen(szSN))
		{
			sp_result = m_pImp->GetShareMemory(ShareMemory_My_UserInputSN, (void*)input_codes, uiSize);
			if (SP_OK != sp_result)
			{
				m_pImp->LogFmtStrW(SPLOGLV_ERROR, L"Get share memory < %s > <%s> failure!", ShareMemory_SN1,ShareMemory_My_UserInputSN);
				m_pImp->NOTIFY("GetShareMemory", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL, "SN1");
				return sp_result;
			}
			if (0 == strlen(input_codes[BC_SN1].szCode))
			{
				m_pImp->LogFmtStrW(SPLOGLV_ERROR, L"GetShareMemory(SN1 Length == 0)");
				m_pImp->NOTIFY("GetShareMemory", LEVEL_ITEM, 1, 0, 1, NULL, -1, NULL, "SN1 Length == 0");
				return SP_E_FAIL;
		...
		{
			input_codes[BC_SN1].bEnable = true;
			strcpy_s(input_codes[BC_SN1].szCode,sizeof(input_codes[BC_SN1].szCode),szSN);
		}
	}
	...
	if (SP_OK != sp_result && 
		(input_codes[BC_SN2].bEnable && (_E_UNISOC_MES_V2 != mes_type || _E_FLOW_CONTROL_SN2 != flow_control)//当SN2走流程的时候，不需要判断SN2
